...
sorted_species <- wide_predictions %>%
  #filter(Change == "Increase") %>%  # Select only "Decrease" species
...
# Save all sorted plots to a single PDF
# pdf("Species_GAM_Plots.pdf", width = 10, height = 8)
...
#write.csv(wide_predictions_with_nsites, "Species_abundance_Predictions_with_Change_and_Real_Change.csv", row.names = FALSE)
...
selected_plots
# Combine the plots into one grid with 3 row and 2 columns
library(cowplot)
combined_plot <- plot_grid(plotlist = selected_plots, nrow = 3, ncol = 2)

# Save the combined plot as a single JPEG image
jpeg("Combined_Species_Plots.jpeg", width = 3000, height = 1000)  # Adjust dimensions as needed
print(combined_plot)
dev.off()
