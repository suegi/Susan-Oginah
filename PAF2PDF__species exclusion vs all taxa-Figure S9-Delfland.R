...
for (g in group_numbers) {
  ...
  abundtox <- Input_data %>%  #filtered_data
    group_by(TWNcode) %>%
    mutate(nsites = n()) %>%
    ungroup()  %>%
    filter(nsites >= 5)
...
plot_gam_exclude <- ggplot(excluded_df, aes(x = delta_msPAF, y = ODF_negative)) +
  ...
  ggtitle("Delfland:Rare and opportunistic species Species excluded")
...
plot_gam_notexclude <- ggplot(not_excluded_df, aes(x = delta_msPAF, y = ODF_negative)) +
  ...
  theme(axis.text.x = element_text(size = 22),
        ...
        plot.title = element_text(size = 30)) +
  scale_y_continuous(limits = c(-0.4, 0.4))+
  ggtitle("Delfland: All species included") 
